...
export const ResultsGrid: React.FC<ResultsGridProps> = ({ results }) => {
    if (results.length === 0) {
        return (
            <div className="bg-white dark:bg-zinc-900 border border-dashed border-gray-300 dark:border-zinc-700 rounded-2xl p-12 text-center text-gray-500">
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((item) => (
                <div key={item.id} className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl overflow-hidden hover:shadow-xl transition-all group flex flex-col">
                    ...
                    <div className="p-5 flex flex-col flex-1 gap-3">
                        <div className="flex justify-between items-start">
                            ...
                            <span className="text-xs text-gray-500 bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded-md line-clamp-1">{new Date(item.datum).toLocaleDateString()}</span>
                        </div>
