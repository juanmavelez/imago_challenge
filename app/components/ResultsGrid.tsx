        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((item) => (
                <li key={item.id} className="group flex h-full">
                    <article className="flex-1 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-2xl overflow-hidden hover:shadow-xl transition-all flex flex-col w-full">
                        ...
                        <div className="p-5 flex flex-col flex-1 gap-3">
                            <header className="flex justify-between items-start">
                                ...
                                <time className="text-xs text-gray-500 bg-gray-100 dark:bg-zinc-800 px-2 py-1 rounded-md line-clamp-1">{new Date(item.datum).toLocaleDateString()}</time>
                            </header>
